...
PidController::PidController(double K_p, double K_i, double K_d, double d_t,
                             double current_state, double target_state) {
  Kp = K_p;
  Ki = K_i;
  Kd = K_d;
  dt = d_t;
...
double PidController::computePidOutput() {
  ...
  while (itr < 250) {
    ...
    pidOutput = proportionalController(currentError) +
                integralController(currentError) +
                derivativeContoller(currentError);
...
/**
 * @brief Function to calculate and return the Derivative error to compute
 * function
 *
 * @param error
 * @return double
...
/**
 * @brief Function to calculate and return the Integral error to compute
 * functions
 *
 * @param error
 * @return double
...
/**
 * @brief Function to calculate and return the Proportional error to compute
 * function
 *
 * @param error
 * @return double
